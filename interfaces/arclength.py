...
class ArcLengthParametrizer(object):
	...
	the new knot vector that you can use to build an arclength reparametrized curve. """
		...
		reparametrization_LS_assembler()
		self.new_knot_vector = reparametrization_LS_solver()
	...
	problem."""
		...
		for i in range(0, s_array[0]):
			tval = FindS(s_array[i])
	...
	will form the arclength reparametrized curve."""
		res = np.linalg.lstsq(self.Bmatrix, self.rhsinit)
